...
var SUPPORTED_AUDIO_FORMATS = ["wav", "mp3", "pcm", "aiff", "aac", "ogg", "wma", "flac", "alac", "mwa"];
...
function buildAnalysisForm() {
  ...
  var template = __dirname.substr(0, getFilenameIndex(__dirname) + 1) + "html/analysisForm.html";
  var form = fs.readFileSync(template, "utf8");
  document.getElementById("analysis-template-holder").innerHTML = form; //Set submit button onclick function

  document.querySelector("#AnalysisForm").addEventListener("submit", submitAnalysis); //Update config and outputFolder
...
function buildOutputTemplate() {
  var template = __dirname.substr(0, getFilenameIndex(__dirname) + 1) + "html/outputTemplate.html";
...
function submitAnalysis(e) {
  ...
  analysisQueue = []; //Build analysis array in reverse order. This is because we pop the array and need to do so by analysis type and file name order.

  for (var analysisType = analysisList.length - 1; analysisType >= 0; analysisType--) {
  ...
  audioFiles = [];
  analysisList = []; //Set progress bar variables to track total analysis

  progressBarMaximum = analysisQueue.length;
  progressBarCurrent = 0; //Update HTML
  ...
  document.querySelector("#output").id = "page"; //Create loading bars with blank analysis
...
function analyse() {
  ...
  var file = currentAnalysis.getSource();
  var filename = getFilename(file);
  filename = filename.substr(0, filename.length - 4);
  var id = generateID(file);
  var analysisType = currentAnalysis.getType();
  updateLoader(id, analysisType); //If the file has not be analysed before, create group to store its data

  if (document.querySelector("#gr" + id) === null) createGroup(id, file); //Get terminal
  ...
  terminal.on("close", function (code) {
    ...
    updateGroup(generateID(currentAnalysis.getSource()), currentAnalysis.getSource(), code === 0, currentAnalysis.getTerminalOutput());
    analyse();
  });
...
function updateTerminalOutput(data) {
  var terminalOutput = document.querySelector("#gr" + generateID(currentAnalysis.getSource()) + " pre");
...
function updateProgressBar(data) {
  var progressreport = "Completed segment"; //Check terminal output for successful environment

  if (data.includes(progressreport)) {
    var PARALLEL_MATCH_LENGTH = 3; //1 full match and 3 groups

    var PARALLEL_REGEX = /INFO.+\/(\d+).+ (\d+) /;
    var SERIAL_REGEX = /INFO.+(\d+)\/(\d+)$/;
    var progress = document.querySelector("#pb" + generateID(currentAnalysis.getSource()));
    var res = PARALLEL_REGEX.exec(data.toString());

    if (res !== null && res.length == PARALLEL_MATCH_LENGTH) {
      var percent = parseInt(parseFloat(res[2]) / parseFloat(res[1]) * 100) + "%";
...
function createGroup(id, filePath) {
  document.querySelector("#output-tab").innerHTML += '<div class="group" id="gr' + id + '"><div class="question" onclick="toggleHeader(this);"><p class="question-text">' + getFilename(filePath) + '</p></div><div class="group-content" style="display: none"id="pic' + id + '"><h1 id="ttl' + id + '" onclick="toggleTerminal(this);">Terminal Output</h1><div class="header-content-padded" style="margin-bottom: -28px;"><div class="output" style="display: none" id="div' + id + '"><pre></pre></div></div></div>';
}
/**
...
function updateGroup(id, fullFilename, success, folder) {
  if (!success) {
    var group = document.querySelector("#pic" + id).parentElement.firstElementChild;
  ...
  fs.readdir(folder, function (err, filenames) {
    ...
    filenames.forEach(function (filename) {
      ...
      if (filename.substr(filename.length - 4) === ".png") {
        ...
        if (filename.substr(getFilenameIndex(filename) + 1, match.length) === match) {
          var title = filename.substr(getFilenameIndex(filename) + 1 + match.length, filename.length - 4);
          title = title.substr(title.lastIndexOf("_") + 1);
          var id = generateID(filename);
          group.innerHTML = '<h1 id="ttl' + id + '" onclick="toggleImage(this);">' + title + '</h1><div class="header-content" id="div' + id + '" style="display: none"><div class="scrollimage" id="img' + id + '"></div></div>' + group.innerHTML;
          buildImageSync(document.getElementById("img" + id), folder + "/" + filename, "", "", "", "");
        }
      }
    });
  });
...
function createLoaders() {
  ...
  var _loop = function _loop(i) {
    var id = generateID(analysisQueue[i].getSource()); //Check if the file already has a group

    processedIDs.some(function (usedID) {
      return usedID === id;
    });
    processedIDs.push(id);
    progressList.push(["<div class='filename-container'>\n\t\t\t\t".concat(getFilename(analysisQueue[i].getSource()), "\n\t\t\t</div>"), "<div class='filename-analysis' align='center' id='an".concat(id, "'>\n\t\t\t\t").concat(analysisQueue[i].getType(), "\n\t\t\t</div>"), "<div class='progress3' id='pb".concat(id, "'>\n\t\t\t\t<div class='cssProgress-bar cssProgress-active-right' style='width: 0%;'>\n\t\t\t\t\t<span class='cssProgress-label'>0%</span>\n\t\t\t\t</div>\n\t\t\t</div>")]);
  ...
  for (var i = analysisQueue.length - 1; i >= 0; i--) {
    _loop(i);
  } //Final push to html
...
function updateLoader(id, analysis) {
  ...
  document.querySelector("#pb" + id).innerHTML = "<div class='cssProgress-bar cssProgress-active-right cssProgress-warning' style='width: 0%;'><span class='cssProgress-label'>0%</span></div>";
}
/**
...
function finishLoader(id, success) {
  ...
  success ? document.querySelector("#pb" + id).innerHTML = "<div class='cssProgress-bar cssProgress-active-right cssProgress-success' style='width: 100%;'><span class='cssProgress-label'>100%</span></div>" : document.querySelector("#pb" + id).innerHTML = "<div class='cssProgress-bar cssProgress-active-right cssProgress-danger' style='width: 100%;'><span class='cssProgress-label'>100%</span></div>";
}

function audio2csvToggle() {
  var advancedOptions = document.querySelector("#audio2csv-options").lastElementChild;
...
function updateAnalyseButton() {
  ...
  if (analysisList.length > 0 && //Check an analysis has been added
...
function loadDefaultOutputFolder() {
  ...
  document.querySelector("#outputFolder li").innerHTML = outputFolder; //Create folder incase it does not exist
...
function setOutputFolder() {
  dialog.showOpenDialog({
    ...
    var content = document.querySelector("#outputFolder .group-content"); //No folder selected

    if (folder !== undefined) {
      outputFolder = folder[0]; //TODO check for write permissions to folder
    }
  ...
  dialog.showOpenDialog({
    ...
    if (files === undefined) {
      ...
      if (audioFiles.length === 0) {
        ...
        document.querySelector("#audio .group-content p").style.display = "none";
        document.querySelector("#audio .group-content ul").style.display = "inherit";
      }
    } else {
      if (files.count == 0) {
        failure("audio");
...
function getAudioFolder() {
  ...
  dialog.showOpenDialog({
    ...
    if (folders === undefined) {
      document.querySelector("#audiospinner").style.display = "none"; //If files have previously been selected
...
function findAudioFiles(folders) {
  ...
  var walk = function walk(dir, done) {
    ...
    fs.readdir(dir, function (err, list) {
      ...
      list.forEach(function (file) {
        file = path.resolve(dir, file); //Determine if file is a directory
...
function updateAudio() {
  ...
  files.innerHTML = "";
  files.innerHTML += "<li><b>Number of Loaded Files: " + audioFiles.length + "</b></li>"; //Storing files and then outputting to html is far faster

  var temp = ""; //Add files to list

  for (var file = 0; file < audioFiles.length; file++) {
    temp += '<li class="files">' + audioFiles[file] + "</li>"; //Push periodically to reduce ram burden
...
function setConfig() {
  ...
  configFiles.forEach(function (file) {
    ...
    option += "<option value=\"".concat(file.getID(), "\"");
    ...
    option += ">".concat(file.getFilename(), "</option>"); //Update default config

    config = file.getFilename() === Defaults.DEFAULT_CONFIG_FILE ? file.getID() : config;
  });
  ...
  walk(Defaults.CONFIG_DIRECTORY, function (err, results) {
    ...
    results.forEach(function (filePath) {
      ...
      if (filePath.substr(filePath.length - 4) === ".yml") {
        ...
        filename = filename.substr(0, filename.length - 4);
        configFiles.push(new ConfigFile(configFiles.length, getFolder(filePath), filename, ".yml"));
      }
    });
    setConfig();
  });
...
function checkEnvironment() {
  ...
  terminal.on("close", function (code) {
    var errorForm = document.querySelector("#environment");
...
function selectAnalysis(el) {
  var inputList = [["audio", false], ["config", false], ["outputFolder", false], ["audio2csv-options", false], ["AnalysisSubmit", false]]; //Check if item exists
  ...
  analysisList.forEach(function (analysisType) {
    switch (analysisType) {
      case "audio2csv":
        addItems(inputList, ["audio", "config", "outputFolder", "audio2csv-options", "AnalysisSubmit"]);
        break;
    }
  }); //Enable required inputs

  inputList.forEach(function (id) {
    if (id[1]) {
...
function failure(id) {
  ...
  if (title !== null) title.setAttribute("class", "question-fail");else return;
  var extra = document.querySelectorAll("#" + id + " a .question-button");
  extra.forEach(function (button) {
...
function success(id) {
  ...
  if (title !== null) title.setAttribute("class", "question");else return;
...
function getTemporaryOutputFolder(id) {
  var checkbox = document.getElementById(id); //If checkbox is checked, repond. Otherwise drop onclick function
