...
function toggleImage(el) {
  ...
  img.style.display === "none" ? img.style.display = "inherit" : img.style.display = "none";
}
/**
...
function toggleHeader(el) {
  ...
  body.style.display === "none" ? body.style.display = "inherit" : body.style.display = "none";
}
/**
...
function toggleInput(el) {
  ...
  if (el.value.includes(":")) {
    //Get list of inputs
    var inputs = el.value.split(":").map(function (input) {
      return "".concat(el.id, "-input").concat(input);
    }); //For each input, if item is checked enable input. Else disable

    inputs.forEach(function (id) {
      el.checked ? document.getElementById(id).disabled = false : document.getElementById(id).disabled = true;
    });
  } else {
    //Single input value
    var id = "".concat(el.id, "-input"); //If item is checked, enable input. Else disable
